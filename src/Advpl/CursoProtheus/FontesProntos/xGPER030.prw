...
//Atilio, 07/10/2021, Constantes que não tem na include atual
#Define STR0216 "Base Dif FGTS"
#Define STR0217 "Base Dif Inss"
#Define STR0237 "| Nome Soc. : "
...
*/
User Function xGPER030(lTerminal,cFilTerminal,cMatTerminal,cProcTerminal, nRecTipo, cPerTerminal, cSemanaTerminal, lMeuRH, cArqName)
...
Local aFldRel		:= {"RA_NOME", "RA_SALARIO", "RA_NOMECMP", "RA_ENDEREC", "RA_COMPLEM", "RA_BAIRRO", "RA_CEP", "RA_MUNICIP", "RA_ESTADO", "RA_CTDEPSA", "RA_EMAIL"}
Local lBlqAcesso	:= aOfusca[2] .And. !Empty( FwProtectedDataUtil():UsrNoAccessFieldsInList( aFldRel ) )
Local cMrhImpBs		:= If( GetMv("MV_IMPBASE",,.T.) , "S", "N" ) //"Para o MEURH, verifica se vai imprimir as verbas de base.
...
Private cSem_De  := "  /  /    "
Private cSem_Ate := "  /  /    "
Private nAteLim , nBaseFgts , nFgts , nBaseIr , nBaseIrFe , nTipRel, nBsDifFgts, nBsDifInss
...
cBaseAux   := IF( !( lTerminal ),If(mv_par21 == 1,"S","N"),If(!lMeuRH,"S",cMrhImpBs) )	//Imprimir Bases
...
If !IsBlind()
	If !Empty(cRotBlank) .And. Len(aPerAberto) == 0 .And. Len(aPerFechado) == 0 
...
ElseIf nTipRel==4 .And. lMeuRH
	...
	cHtml := R030Imp(.F.,wnRel,cString,cMesAnoRef,lTerminal,lMeuRH,cArqName)
	If Select("SRC")>0
		SRC->(DbCloseArea())
	EndIf
Else
...
*/
Static Function R030Imp(lEnd,WnRel,cString,cMesAnoRef,lTerminal,lMeuRH,cArqName)
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Define Variaveis Locais (Basicas)                            ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
//Local aCodBenef   	:= {}
...
Local cPath			:= supergetmv("MV_RELT",,"\spool\") + "holerite\"
...
If lTerminal
	...
	If !Eof()
		While RCH->( !Eof() .and. &(cInicio) == &(cKey))
			...
			If MesAno(dDataRef) == RCH->RCH_ANO+RCH->RCH_MES
	...
	If	(cPerCorrente == cPeriodo .AND. cSemCorrente == Semana)  .OR. ;
		...
		If  cTipoRot == "2" //Adiantamento
			If ( If(MesAno(Date()) == (cAnoCorrente + cMesCorrente), Day(Date()) < nTCFDADT, .F.) )
				Return( IF( lTerminal <> NIL .And. lTerminal , cHtml , NIL ) )
			EndIf
		ElseIf !lMeuRH .And. (cTipoRot == "1" .Or. cTipoRot == "9" ).and. !empty(nTCFDFOL) //Folha
...
While SRA->( !Eof() .And. &cInicio <= cFim )
	...
	nAteLim := nBaseFgts := nFgts := nBaseIr := nBaseIrFe := nBsDifFgts := nBsDifInss := 0.00
	...
	If cSitFunc == "D" .And. ( (!Empty(SRA->RA_DEMISSA) .And. MesAno(SRA->RA_DEMISSA) > MesAno(dDtPesqAf)) .Or. ( cRoteiro == fGetCalcRot("F") .And. GetMvRH("MV_PLRDEM",,"N") == "S" .AND. cSitFunc $ cSituacao ) )
		cSitFunc := " "
	Endif
	...
	If SRA->RA_Filial # cFilialAnt
		If ( ! Fp_CodFol(@aCodFol,Sra->Ra_Filial,,!lMeuRH) .And. !lMeuRH ) .Or. ! fInfo(@aInfo,Sra->Ra_Filial)
			Return Nil
		Endif
	...
	If !cPaisLoc $ "CHI|PAR|DOM|BOL|" .AND. !lFase4
	...
	If cRoteiro <> "EXT"
		For nReg := 1 to Len(aVerbasFunc)
			...
			Elseif (aVerbasFunc[nReg,3] == aCodFol[0338,1])
				nBsDifInss += aVerbasFunc[nReg,7]
			Elseif (aVerbasFunc[nReg,3] == aCodFol[0337,1])
				nBsDifFgts += aVerbasFunc[nReg,7]
			Endif
		Next nReg
	...
	ElseIf (nTipRel == 4 .OR. nTipRel == 5) .and. (!( lTerminal ) .Or. lMeuRH) //4 Visualizar o PDF 5 Enviar PDF por email
		fImprPDF(lPDFEmail, cPath, lMeuRH, cArqName)
		ASize(AProve,0)
		ASize(ADesco,0)
		ASize(aBases,0)
	Endif
...
Local nConta    := 0 //nContr := nContrT:=0
...
cNomeSoc := fTAcento(Left(SRA->RA_NSOCIAL, 28))
cNomeSoc := fTrocaCar(cNomeSoc)
...
If !Empty(SRA->RA_DEPTO)
	...
	@ LI,58 PSAY ALLTRIM(cCodCC)
Else
	@ LI,53 PSAY PADC(ALLTRIM(cCodCC),12)
EndIf
...
LI ++
@ LI,00  PSAY STR0024 + If(lLpt1, fTAcento(SRA->RA_NOME), SRA->RA_NOME )	//"| Nome  : "
@ LI,131 PSAY "|"

LI ++
@ LI,00  PSAY STR0237 + If(lLpt1, fTAcento(SRA->RA_NSOCIAL), SRA->RA_NSOCIAL)	//"| Nome Soc. : "
@ LI,131 PSAY "|"

LI ++
@ LI,00   PSAY STR0026+cCodFunc+" - "+If( lLpt1, fTAcento(cDescFunc), cDescFunc ) //"| Funcao    : "
If !Empty(SRA->RA_DEPTO)
	@ LI,46   PSAY STR0177 + ": " + SRA->RA_DEPTO + " - " + fDesc('SQB',SRA->RA_DEPTO,'QB_DESCRIC',,SRA->RA_FILIAL)	//-9caracteres+30caracteres
EndIf
...
Static Function fCabecG()   // Cabecalho do Recibo Zebrado
	...
	oPrint:Say(0250,1950,STR0148+cCodCC,oArial08)												//Centro de Custo
	oPrint:Say(0310,1015,Alltrim(SRA->RA_BAIRRO)+"-"+Alltrim(SRA->RA_CEP),oArial08)					//Bairro e Cep
	oPrint:Say(0310,1950,STR0149+cDescCC,oArial08)						//Descricao Centro de Custo
	IF cPaisloc == "COL"
...
If ( Ascan(aProve , {|aProve| aProve[3] >= 100000} ) > 0 )
...
IF !( lTerminal )
	...
	If cPaisloc <> "PER"
		...
		// NOME FUNCIONARIO
		cHtml +=   '<font color="#000082" face="Courier New"><i><b>'
		cHtml +=  	'&nbsp;&nbsp;&nbsp;'  + "Nome: " + SRA->RA_NOME + " - " + SRA->RA_MAT + '</i></b></font><br>'
		cHtml += 	'<font color="#000082" face="Courier New" size="2">'

		// NOME SOCIAL FUNCIONARIO
		cHtml +=   '<font color="#000082" face="Courier New"><i><b>'
		cHtml +=  	'&nbsp;&nbsp;&nbsp;'  + "Nome Soc.: " + SRA->RA_NSOCIAL + '</i></b></font><br>'
		cHtml += 	'<font color="#000082" face="Courier New" size="2">'
		
		cHtml += 	'&nbsp;&nbsp;&nbsp;&nbsp;' + STR0048 + cCodFunc + "  "+cDescFunc	+'<br>' //"Funcao    - "
		cHtml +=  	'&nbsp;&nbsp;&nbsp;&nbsp;' + STR0047 + cCodCC + " - " + cDescCc +'<br>' //"C.Custo   - "
	...
	IF cBaseAux = "S"
		...
		IF !( lTerminal )
			...
			if(nBsDifFgts > 0)
				cHtml += '<tr>'
				cHtml +=	'<td class="tdPrinc">'
				cHtml +=    '<p class="pStyle1">'+ STR0216 +'</p></td>'//"Base Dif FGTS"
				cHtml +=	'<td align="right"><font face="Arial" size="02" color="#000082"><b>' + Transform(nBsDifFgts,cPict3)+ '</b></font></td>'
				cHtml += '</tr>'
			EndIf
			if(nBsDifInss > 0)
				cHtml += '<tr>'
				cHtml +=	'<td class="tdPrinc">'
				cHtml +=    '<p class="pStyle1">'+ STR0217 +'</p></td>'//"Base Dif Inss"
				cHtml +=	'<td align="right"><font face="Arial" size="02" color="#000082"><b>' + Transform(nBsDifInss,cPict3)+ '</b></font></td>'
				cHtml += '</tr>'
			EndIf
		Else
			...
			if(nBsDifFgts > 0)
				cHtml += 								"<tr>" + CRLF
				cHtml += 									"<td width='219' class='etiquetas' bgcolor='#FAFBFC' >"
				cHtml +=										STR0216 + CRLF //"Base Dif FGTS"
				cHtml += 									"</td>" + CRLF
				cHtml += 									"<td width='45' class='dados'>" + CRLF
				cHtml += 										'<div align="left" class="etiquetas" bgcolor="#FAFBFC"> </div>' + CRLF
				cHtml +=									"</td>" + CRLF
			...
			if(nBsDifInss > 0)
				cHtml += 								"<tr>" + CRLF
				cHtml += 									"<td width='219' class='etiquetas' bgcolor='#FAFBFC' >"
				cHtml +=										STR0217 + CRLF //"Base Dif Inss"
				cHtml += 									"</td>" + CRLF
				cHtml += 									"<td width='45' class='dados'>" + CRLF
				cHtml += 										'<div align="left" class="etiquetas" bgcolor="#FAFBFC"> </div>' + CRLF
				cHtml +=									"</td>" + CRLF
...
If dbSeek(cFilial)
	...
	While !Eof() .And. SRY->RY_FILIAL == cFilial
		If !(SRY->RY_TIPO $ (IIF( (cPaisLoc=="PAR"), "G*J","3*4*G*J")))
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß*/
Static Function fImprPDF(lPDFEmail,cPath, lMeuRH, cArqName)
Local nConta 		:= 0
Local cFile		:= ""
Local cEmail		:= ""
Local cIdentUni	:= ""
...
If lPDFEmail .And. !lMeuRH
    ...
    EndIf
ElseIf lMeuRH
	cPath := SuperGetMV("MV_RELT", , "\spool\") + "holerite\" //GetSrvProfString("STARTPATH","")
	oPrinter	:= FWMSPrinter():New( cArqName, 6,.F.,cPath,.T.,,,,.T.,.F.)
EndIf
...
@ LI,40  PSAY STR0132 + SRA->RA_DEPTO + " - " + Trim(PADR(fDesc('SQB',SRA->RA_DEPTO,'QB_DESCRIC',,SRA->RA_FILIAL),30,""))	//-9caracteres+30caracteres
...
Static Function fCabPDF(oPrinter)
	...
	nLi += 10
	oPrinter:Box(nLi + 5, 004, nLi + 125, 585, "-4" )			// Caixa da linha
	nLi += 20

	cNomePadr := iif(lCorpManage,"lgrl"+ FWGrpCompany()+ SRA->RA_FILIAL + ".BMP","lgrl" + SRA->RA_FILIAL + ".BMP" )
	cStrLogo    := RetLogoEmp(.F.,, SRA->RA_FILIAL, "lgrl", .T. )

	If(cNomePadr == cStrLogo) .Or. File(cStrLogo)
		nLx := 080
		oPrinter:SayBitmap(nLi, 005, cStartPath + cStrLogo, nLx, 040) // Tem que estar abaixo do RootPath
	EndIf
	oPrinter:Say(nLi,nLx,Space(3)+Substr(STR0018, 3) +  DESC_Fil,oFont2N)	//"| Empresa   : "
	...
	nLi += 10
	cDescCC := If( lLpt1, fTAcento(cDescCC),cDescCC)
	oPrinter:Say(nLi, nLx,Space(3)+Substr(STR0020, 3) + cCodCC + " - " + cDescCC,oFont2N)	//"| C Custo   : "
	...
	nLi += 10
	oPrinter:Say(nLi,nLx,Space(3)+Substr(STR0023,3) + SRA->RA_MAT + "   " + StrTran(STR0024, "|", "") + If( lLpt1, fTAcento(SRA->RA_NOME), SRA->RA_NOME ),oFont2N)	//"| Matricula : " XXXXX +"   " + Nome  : " XXXXXXXXXXXXXXX
	oPrinter:Say(nLi,447,STR0019 + SRA->RA_FILIAL,oFont2N)	//" Local : "

	nLi += 10
	oPrinter:Say(nLi, nLx, Space(3) + Alltrim(StrTran(STR0237, "|", "")) + " " + If(lLpt1, fTAcento(SRA->RA_NSOCIAL), SRA->RA_NSOCIAL), oFont2N)	// "| Nome Soc. : "

	nLi += 10
	oPrinter:Say(nLi,nLx,Space(3)+Substr(STR0026,3)+cCodFunc+" - "+If( lLpt1, fTAcento(cDescFunc), cDescFunc ),oFont2N)	//"| Funcao    : "
	If !Empty(SRA->RA_DEPTO)
		oPrinter:Say(nLi,260,STR0177 + ": " + SRA->RA_DEPTO + " - " + fDesc('SQB',SRA->RA_DEPTO,'QB_DESCRIC',,SRA->RA_FILIAL),oFont2N)	//-9caracteres+30caracteres
	EndIf
	...
	oPrinter:Say(nLi,nLx,Space(2) + STR0213 + "  " + Alltrim(Transform(nSalario,cPict2)),oFont2N)	//Salario
...
Local nSpace    	:= 0
Local cMsgAniv  	:= ""
Local nPosFCab	:= 124 //Posição final do cabeçalho fCabPDF()
Local nLi			:= nPosFCab
Local nTotProv		:= 0

	aEval(aProve , {|aProve| nTotProv += aProve[3] } )
	If ( nTotProv >= 1000000 )
...
Local cPath	:= Lower(supergetmv("MV_RELT",,"\spool\")) + "holerite\"
...
Static Function fCabecZDom()   // Cabecalho do Recibo Zebrado
	...
	Local cFechaPer := DTOC(Posicione("RCH", 1 , xfilial("RCH")+cProcesso+cPeriodo+cSemana+cRoteiro , "RCH_DTINI")) +;
		...
		@ LI,60  PSAY STR0132 + IIF(!Empty(SRA->RA_DEPTO),ALLTRIM(SRA->RA_DEPTO) + " - " + ALLTRIM(fDesc('SQB',SRA->RA_DEPTO,'QB_DESCRIC',,SRA->RA_FILIAL)),UPPER(STR0206)) 	//"DPTO.: "
		@ LI,131 PSAY "|"
